...
# set tabkey [ lindex [chg_tabkey $tabkey] 1] ; set i 12
# source [ file join $audace(rep_plugin) tool bddimages site eso-vlt-u4.tcl]
# ::bddimages_liste::lget $tabkey ESO_DET_DIT
# set tabkey [::bddimages_liste::ldelete $tabkey EXPOSURE]
# --------------------------------
...
global audace 
   ...
   if {! [::bddimages_liste::lexist $tabkey EXPOSURE]} {
      set line [::bddimages_liste::lget $tabkey EXPTIME]
      set exposure [lreplace $line 0 0 EXPOSURE]
      set exposure [lreplace $line 4 4 "sec"]
   ...
   if {! [::bddimages_liste::lexist $tabkey "bin1"]} {
      if {[::bddimages_liste::lexist $tabkey "binx"]} {
         set cle [::bddimages_liste::lget $tabkey "binx"]
         set bin [lreplace $cle 0 0 "bin1"]
         set tabkey [::bddimages_liste::ladd $tabkey "bin1" $bin]
      } else {
         set bin 1
         set tabkey [::bddimages_liste::ladd $tabkey "bin1" [list BIN1 $bin int "Binning in X coordinate" ""]]
      }
   }
   ...
   if {! [::bddimages_liste::lexist $tabkey "bin2"]} {
      if {[::bddimages_liste::lexist $tabkey "biny"]} {
         set cle [::bddimages_liste::lget $tabkey "biny"]
         set bin [lreplace $cle 0 0 "bin2"]
         set tabkey [::bddimages_liste::ladd $tabkey "bin2" $bin]
      } else {
         set bin 1
         set tabkey [::bddimages_liste::ladd $tabkey "bin2" [list BIN2 $bin int "Binning in Y coordinate" ""]]
      }
   }
   ...
   if {! [::bddimages_liste::lexist $tabkey "filter"]} {
      if {[::bddimages_liste::lexist $tabkey "fltrnm"]} {
         set cle [::bddimages_liste::lget $tabkey "fltrnm"]
         set f [lreplace $cle 0 0 "filter"]
   ...
   set newtabkey  ""
   foreach info $tabkey {
      set name [bddimages_keywd_to_variable [lindex $info 0]]
      if {[string first "eso" $name] >= 0} {
         gren_info "Rejetee : ($name) - ([lindex $info 1 0])\n"
         buf$audace(bufNo) delkwd [lindex $info 1 0]
         continue
      } else {
         lappend newtabkey $info
      }
   }
   set tabkey $newtabkey
