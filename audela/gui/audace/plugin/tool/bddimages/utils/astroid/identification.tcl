...
if {! [string is double $serrpos]} {
   gren_erreur "errpos = $errpos\n"
   return false
}
if {! [string is double $ierrpos]} {
   gren_erreur "ierrpos = $ierrpos\n"
   return false
}
if {! [string is double $irmag]} {
   gren_erreur "irmag = $irmag\n"
   return false
}
if {! [string is double $srmag]} {
   gren_erreur "srmag = $srmag\n"
   return false
}
if {! [string is double $irmagerr]} {
   gren_erreur "irmagerr = $irmagerr\n"
   return false
}
if {! [string is double $srmagerr]} {
   gren_erreur "srmagerr = $srmagerr\n"
   return false
}
...
proc identification { catalist1 catalog1 catalist2 catalog2 scoreposlimit scoremvlimit f log {threadId ""} } {
...
# Cross-match
   ...
   if {$threadId==""} {
      ...
      # gren_info "[gren_date] SLAVE \[$threadId\]:      identification: CrossMatch $catalog1 VS $catalog2 in $tt secondes for $nbralist1 x $nbralist2 sources ...(P$scoreposlimit|M$scoremvlimit) Matched : [llength $ident]\n"
  }
  return $result
