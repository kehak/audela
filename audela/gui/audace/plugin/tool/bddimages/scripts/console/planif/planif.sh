...
if [ $# -lt 1 ]; then
   if [ -f "${AUDELA_CNF}/planif.ini"  ]; then
      printf "Utilisation de la conf ${AUDELA_CNF}/planif.ini\n"
   else
      printf "${AUDELA_CNF}/planif.ini n'existe pas, voulez vous en creer un ? (y/N)\n"
      read a
      if [ "x${a}" == "xy" ]; then
          cp $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/planif/planif.ini ${AUDELA_CNF}/planif.ini
          printf "${AUDELA_CNF}/planif.ini cree, veuillez le completer...\n"
          exit 0
      fi
   ...
   cp $1 ${AUDELA_CNF}/planif.ini
fi

# On tue toutes les instances pouvant exister
for pid in `ps -edf | grep planif.tcl | awk '{print $2}'`
do 
   echo "PID = ${pid}"
   kill -9 ${pid}
done
...
$AUDELA_DIR/bin/audela --console --file $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/planif/planif.tcl
err=$?
echo "PLANIF : Sortie Audela : ERR = ${err}"
