...
if [ $# -lt 1 ]; then
   if [ -f "${AUDELA_CNF}/diabolo.ini"  ]; then
      printf "Utilisation de la conf ${AUDELA_CNF}/diabolo.ini\n"
   else
      printf "${AUDELA_CNF}/diabolo.ini n'existe pas, voulez vous en creer un ? (y/N)\n"
      read a
      if [ "x${a}" == "xy" ]; then
          cp $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/diabolo/diabolo.ini ${AUDELA_CNF}/diabolo.ini
          printf "${AUDELA_CNF}/diabolo.ini cree, veuillez le completer...\n"
          exit 0
      fi
   ...
   cp $1 ${AUDELA_CNF}/diabolo.ini
fi

$AUDELA_DIR/bin/audela --console --file $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/diabolo/diabolo.tcl
