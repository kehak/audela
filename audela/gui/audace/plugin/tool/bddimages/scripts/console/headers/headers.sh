...
if [ $# -lt 1 ]; then
   if [ -f "${AUDELA_CNF}/headers.ini"  ]; then
      printf "Utilisation de la conf ${AUDELA_CNF}/headers.ini\n"
   else
      printf "${AUDELA_CNF}/headers.ini n'existe pas, voulez vous en creer un ? (y/N)\n"
      read a
      if [ "x${a}" == "xy" ]; then
          cp $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/headers/headers.ini ${AUDELA_CNF}/headers.ini
          printf "${AUDELA_CNF}/headers.ini cree, veuillez le completer...\n"
          exit 0
      fi
...
do
   ...
   for pid in `ps -edf | grep headers.tcl | awk '{print $2}'`
   do 
      echo "PID = ${pid}"
      kill -9 ${pid}
   done
   ...
   count=$(( $count + 1 ))
   $AUDELA_DIR/bin/audela --console --file $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/headers/headers.tcl
   err=$?
   echo "Headers : Sortie Audela : ERR = ${err}"
   ...
   echo `hostname` >> out
   echo `uname -a` >> out
   cat out | mail -s "Audela - Bddimages - Headers : `hostname` STOPPED `date`" $AUDELA_MAIL
   echo "Sleep $tps Sec"
   echo "----------------------------------------"
   sleep $tps
done
