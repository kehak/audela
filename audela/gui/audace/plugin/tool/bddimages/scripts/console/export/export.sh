...
if [ $# -lt 1 ]; then
   if [ -f "${AUDELA_CNF}/export.ini"  ]; then
      printf "Utilisation de la conf ${AUDELA_CNF}/export.ini\n"
   else
      printf "${AUDELA_CNF}/export.ini n'existe pas, voulez vous en creer un ? (y/N)\n"
      read a
      if [ "x${a}" == "xy" ]; then
          cp $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/export/export.ini ${AUDELA_CNF}/export.ini
          printf "${AUDELA_CNF}/export.ini cree, veuillez le completer...\n"
          exit 0
      fi
   ...
   cp $1 ${AUDELA_CNF}/export.ini
fi

$AUDELA_DIR/bin/audela --console --file $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/export/export.tcl
