...
if [ $# -lt 1 ]; then
   if [ -f "${AUDELA_CNF}/import.ini"  ]; then
      printf "Utilisation de la conf ${AUDELA_CNF}/import.ini\n"
   else
      printf "${AUDELA_CNF}/import.ini n'existe pas, voulez vous en creer un ? (y/N)\n"
      read a
      if [ "x${a}" == "xy" ]; then
          cp $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/import/import.ini ${AUDELA_CNF}/import.ini
          printf "${AUDELA_CNF}/import.ini cree, veuillez le completer...\n"
          exit 0
      fi
