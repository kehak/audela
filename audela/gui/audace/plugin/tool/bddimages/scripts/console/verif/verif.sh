...
if [ $# -lt 1 ]; then
   if [ -f "${AUDELA_CNF}/verif.ini"  ]; then
      printf "Utilisation de la conf ${AUDELA_CNF}/verif.ini\n"
   else
      printf "${AUDELA_CNF}/verif.ini n'existe pas, voulez vous en creer un ? (y/N)\n"
      read a
      if [ "x${a}" == "xy" ]; then
          cp $AUDELA_DIR/gui/audace/plugin/tool/bddimages/scripts/console/verif/verif.ini ${AUDELA_CNF}/verif.ini
          printf "${AUDELA_CNF}/verif.ini cree, veuillez le completer...\n"
          exit 0
      fi
