/*
 *
 *	xclibsv.h	External	30-Jul-2014
 *
...
/*
 * Library version ID's  (Internal Note: UpdateFlag SFUID!)
 */
#include "xclibver.h"


/*
 * PIXCI(R) frame grabber model codes.
 */
...
struct xcdevinfo1 {
    ...
    /*
     */
    uint16  fpgalevel;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8CAM/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2 */
    uint16  pcblevel;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8CAM/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2*/
    uint16  cam0code;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8CAM/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2*/
    uint16  cam1code;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8CAM/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2*/
    uint16  sibid;		/* added 30-Dec-2011 */
    uint16  rsvd[15];
    uint32  rsvd2[16];
...
struct xcdevinfo {
     ...
     */
    uint16  fpgalevel;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2 */
    uint16  pcblevel;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2 */
    uint16  cam0code;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2 */
    uint16  cam1code;		/* for PIXCI(R) A310/CL2/E1/E4/E8/EB1/EC1/EL1/E1DB/EL1DB/ELS2/E8CAM/E4DB/E8DB/SI1/SI2/SI4/SV7/SV8/e104x4/EB1Mini/E4G2 */
    uint16  sibid;		/* added 30-Dec-2011 */
    uint16  rsvd[15];
    uint32  rsvd2[16];
...
struct xcdevparms {
    ...
    uint8   memmap;		/* -MU:  0x1: map memory to user space? 		*/
				...
				/* -MU:  0x8: buffer status in system memory, instead	*/
				/*	      of frame buffer memory			*/

    uint32  rsvd32[31];
    uint32  serialbufsize;	/* -SB: size of serial data buffers			*/
    uint16  rsvd16[32];
    uint8   rsvd8[30];
    uint8   timer;		/* -TI:  0: disable time stamping, otherwise		*/
				/*	 select system timer service to be used 	*/
    uint8   bufstat;		/* -BS:  enable per-frame buffer capture status 	*/
};
...
struct xcvidstatus2 {
	...
	struct sv7 {
	    ...
	    uint8	line525;		/* 1: video is 525 line, 0: is 625 line */
	} sv7;
	struct a310 {
	    int 	vdrive2;
	    int 	hdrive2;
	    uint16	linesperframe;
	    uint16	clocksperline;
	    uint16	alcoutput[3];
	    uint8	hsyncwidth;
	    uint8	vsyncwidth;
	    uint8	macrovisiondetected;
	    uint8	interlacedetected;
	    uint8	hsyncdetect;
	    uint8	hsyncactive;
	    uint8	hsyncpolarity;
	    uint8	vsyncdetect;
...
/*
 * PIXCI(R) frame grabber low level services
 */
...
/*
 * PIXCI(R) frame grabber library level services
 */
...
_cDcl(_dllxxxxx,_cfunfcc,int)	xclib_open (struct xclibs *xclib,const char *modes,const char *driverparms,const char *formatname,const char *formatfile);
_cDcl(_dllxxxxx,_cfunfcc,int)	xclib_close(struct xclibs *xclib);
#else
_cDcl(_dllpxlib,_cfunfcc,int)	xclib_open (struct xclibs *xclib,const char *modes,const char *driverparms,const char *formatname,const char *formatfile);
 ...
 *
 * The pxcamcntl is now variable length; this
 * provides the fixed length portion for maximum backward
 * compatibility, rather than not providing the pxcamcntl.
 */
#define xclib_DeclareVidStateStructs(Name)  xclib_DeclareVidStateStructs2(Name, PIXCI_SV2)


#define xclib_InitVidStateStructs(Name) xclib_InitVidStateStructs2(Name, PIXCI_SV2)
...
#define xclib_DeclareVidStateStructs2(Name, Model)    \
	    ...
	    struct pxvidmem	    Name_##vidmem;	 \
	    struct pxcamcntl	    Name_##camcntl;	 \
	    union  xcanalogformat   Name_##anaformat;	 \
	    union  xcanalogmode     Name_##anamode;	 \
	    struct xcdxxformat	    Name_##dxxformat;
#define xclib_InitVidStateStructs2(Name, Model)        \
	    Name.vidformat    = &Name_##vidformat;     \
	    Name.vidres       = &Name_##vidres;        \
	    ...
	    Name.camcntl      = &Name_##camcntl;       \
	  /*Name.camcntl      = NULL; */	       \
	    Name.xc.anaformat = &Name_##anaformat;     \
	    Name.xc.anamode   = &Name_##anamode;       \
	    Name.xc.dxxformat = &Name_##dxxformat;     \
	    Name	    .ddch.len = sizeof(Name);		   Name.ddch.mos	     = PXMOS_VIDSTATE;	   \
	    Name_##vidformat.ddch.len = sizeof(Name_##vidformat);  Name_##vidformat.ddch.mos = PXMOS_VIDFORMAT;    \
	    Name_##vidres   .ddch.len = sizeof(Name_##vidres);	   Name_##vidres.ddch.mos    = PXMOS_VIDRES;	   \
	    ...
	    Name_##vidmem   .ddch.len = sizeof(Name_##vidmem);	   Name_##vidmem.ddch.mos    = PXMOS_VIDMEM;	   \
	    Name_##camcntl  .ddch.len = sizeof(Name_##camcntl);    Name_##camcntl.ddch.mos   = PXMOS_CAMCNTL;	   \
	    if (Model == PIXCI_A310 || Model == PIXCI_A110) { \
		Name_##anaformat.a310format.ddch.len = sizeof(Name_##anaformat.a310format);  Name_##anaformat.a310format.ddch.mos = XCMOS_A310FORMAT; \
		Name_##anamode.a310mode    .ddch.len = sizeof(Name_##anamode.a310mode);      Name_##anamode.a310mode	.ddch.mos = XCMOS_A310MODE; \
	    } else if (Model == PIXCI_SV8) { \
		Name_##anaformat.sv8format.ddch.len = sizeof(Name_##anaformat.sv8format);  Name_##anaformat.sv8format.ddch.mos = XCMOS_SV8FORMAT; \
		Name_##anamode.sv8mode	  .ddch.len = sizeof(Name_##anamode.sv8mode);	   Name_##anamode.sv8mode    .ddch.mos = XCMOS_SV8MODE; \
	    } else { \
		Name_##anaformat.sv2format.ddch.len = sizeof(Name_##anaformat.sv2format);  Name_##anaformat.sv2format.ddch.mos = XCMOS_SV2FORMAT; \
		Name_##anamode.sv2mode	  .ddch.len = sizeof(Name_##anamode.sv2mode);	   Name_##anamode.sv2mode    .ddch.mos = XCMOS_SV2MODE; \
	    } \
	    Name_##dxxformat.ddch.len = sizeof(Name_##dxxformat);  Name_##dxxformat.ddch.mos = XCMOS_DXXFORMAT;
#endif
