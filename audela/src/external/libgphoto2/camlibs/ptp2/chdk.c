...
chdk_get_iso(CONFIG_GET_ARGS) {
	...
	if (!retint) {
		...
		iso = (int)(_exp2(retint/96.0)*3.125);
	} else {
		iso = retint;
	}
...
chdk_init(Camera *camera, GPContext *context) {
   ...
   //modif michel
   chdk_fsfuncs.file_list_func = chdk_file_list_func;
	chdk_fsfuncs.folder_list_func = chdk_folder_list_func;
	chdk_fsfuncs.get_info_func = chdk_get_info_func;
	chdk_fsfuncs.get_file_func = chdk_get_file_func;
	chdk_fsfuncs.del_file_func = chdk_delete_file_func;
