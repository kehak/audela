...
void CFile::loadFits(const char * fileName, int iaxis3, int dataTypeOut, CPixels **pixels, CFitsKeywords **keywords)
{
   ...
   try {
      ...
      *keywords = new CFitsKeywords();
      ...
      CFitsKeyword * k = (*keywords)->FindKeyword("NAXIS");
      if(k == NULL  ) {
      ...
      if ( naxis == 3 && naxis3 == 3 && iaxis3 == 0 ) {
         ...
         *pixels = new CPixelsGray(naxis1, naxis2, pixelFormat, ppix, 0, 0);
         ...
         CFitsKeyword * k = (*keywords)->FindKeyword("NAXIS3");
         if(k != NULL  ) {
...
void CFile::saveFits(const char * fileName, int dataTypeOut, CPixels *pixels, CFitsKeywords *keywords)
{
   int msg;                         // Code erreur de libtt
...
void CFile::saveFitsTable(const char * outputFileName, CFitsKeywords *keywords, int nbRow, int nbCol, char *columnType, char **columnTitle, char **columnUnits, char **columnData )
{
...
void CFile::saveJpeg(const char * fileName, unsigned char *dataIn, CFitsKeywords *keywords, int planes,  int width, int height, int quality)
{
