...
int cmdRestoreInitialCut(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
...
int cmdUnifyBg(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdRegion(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
...
int cmdAutocuts(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);

int cmdDMTestVariance(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
...
//
int CmdBuf(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	char s[2048],ss[50];
	int retour = TCL_OK,k;
	struct cmditem *cmd;
				...
				if( strcmp(ext, ".fit")== 0
					...
					if (Buffer->GetCompressType()==BUFCOMPRESS_GZIP) {
						...
						if( Tcl_Eval(interp,ligne) != TCL_OK  ) {
							throw CError("gzip %s",Tcl_GetStringResult(interp));
						} else {
							// je verifie que gzip retourn "1"
							if(strcmp(Tcl_GetStringResult(interp),"0")!=0) {
								throw CError("gzip %s returns %s : Intermediate file not found ",nom_fichier,Tcl_GetStringResult(interp));
							}
						}
					}
...
int cmdTtSub(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	if (retour== TCL_OK) {
		...
		if(Tcl_GetInt(interp,argv[2],&subBufNo)==TCL_OK) {
			...
			try {
				buffer = (CBuffer*)clientData;
				CBuffer * subBuf = (CBuffer*)buf_pool->Chercher(subBufNo);
	...
	if(argc!=19) {
		...
		if ( retour == TCL_OK ) {
			...
			try {
				...
				strcpy(message,"");
				CBuffer* maskBuf = (CBuffer*)buf_pool->Chercher(maskBufNo);
				CBuffer* sumBuf = (CBuffer*)buf_pool->Chercher(sumBufNo);
