...
// Factory statique et publique 
CBuffer* CBuffer::createBuffer() {


   CBuffer *newBuffer = new CBuffer();
   
   if( buf_pool->Ajouter(0,newBuffer) )  {
      return newBuffer;
   } else {
      throw CError("create buffer error");
   }

}

void CBuffer::deleteBuffer(CBuffer* buffer) {
...
void CBuffer::Save3d(char *filename,int naxis3,int iaxis3_beg,int iaxis3_end)
{
   ...
   if ((naxis3<=0)||(nnaxis2<1)) {
      ...
      //if ( ppix+ naxis1*nnaxis2*nnaxis3 > ppix1 + ppix1Length) {
      //   libtt_main(TT_PTR_FREEKEYS,5,&keynames,&values,&comments,&units,&datatypes);
      //   free(ppix1);
      //   throw CError("depassement de buffer");
      //}
...
void CBuffer::setPixels(int width, int height, int plane, TYPE_PIXELS * pixels, int reverseX, int reverseY) {
   ...
   if ( plane == 1) {
      ...
      throw CError("setPixels plane=%d. plane must be 1 or 3", plane);
   }
...
int CBuffer::getKeywordDataType(const char *keywordName) {
   ...
   if ( kwd != NULL ) {
      ...
      throw CError(ELIBSTD_KWD_NOT_FOUND); 
   }
}
...
void CBuffer::Sub(CBuffer* subBuffer , float offset)
{
   if (subBuffer == NULL ) {
      throw CError( "CBuffer::Sub subBuffer=NULL");
   }
...
// -----------------------------------------------------------------
void CBuffer::p2radec(double p1, double p2, double *ra, double *dec,int order)
{
	CBuffer::Header2Wcs();
   if(wcs.valid==0) {
      throw CError(ELIBSTD_NO_ASTROMPARAMS);
   }
	double x1,x2,phi,rth,theta,sinth,costh,sindec,cosdec,cosdecsinrarap,cosdeccosrarap,rarap;
	p1 = p1 +1.;
	p2 = p2 +1.;
...
// -----------------------------------------------------------------
// Attention, ra,dec en radian
void CBuffer::radec2p(double ra, double dec,double *p1, double *p2, int order)
{
	CBuffer::Header2Wcs();
   if(wcs.valid==0) {
      throw CError(ELIBSTD_NO_ASTROMPARAMS);
   }
	double cosdec,sindec,sinth,costh,costhsinphiphip,costhcosphiphip,phiphip,phi,theta;
