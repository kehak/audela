...
int cmdTelPosition(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   tel = (struct telprop *)clientData;
	res=autostar_position_tube(tel,sens);
	if (res==0) {
...
int cmdTelPut(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   if (argc<3) {
      ...
      res=autostar_put(tel,argv[2]);
      if (res!=0) {
			sprintf(ligne,"Error number %d",res);
         result = TCL_ERROR;
      } else {
			sprintf(ligne,"%d",res);
         Tcl_SetResult(interp,ligne,TCL_VOLATILE);
...
int cmdTelPutread(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   if (argc<3) {
      ...
      result = TCL_ERROR;
		...
		res=autostar_putread(tel,argv[2],ligne);
      if (res!=0) {
...
int cmdTelStatus(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   struct telprop *tel;
   tel = (struct telprop *)clientData;
   res=autostar_stat(tel,ligne,bits);
   if (res==1) {
      result = TCL_ERROR;
   } else {
      Tcl_SetResult(interp,ligne,TCL_VOLATILE);
