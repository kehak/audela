...
struct telini tel_ini[] = {
   {"ASTELCO",    /* telescope name */
    "Astelco",    /* protocol name */
    "astelco",    /* product */
...
/* --------------------------------------------------------- */
{
   ...
   tel->tempo=50;
#if defined astelco_MOUCHARD
   f=fopen("mouchard_astelco.txt","wt");
   fclose(f);
#endif
...
#if defined astelco_MOUCHARD
	f=fopen("mouchard_astelco.txt","at");
	fprintf(f,"IP=%s\n",tel->ip);
	fclose(f);
#endif
	/* --- decode port  --- */
   ...
   sprintf(s,"after 350"); mytel_tcleval(tel,s);
	/* --- VERSION --- */
   res=astelco_put(tel,"VERSION");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_read(tel,s);
	/* --- TELESCOPE CONFIG --- */

	//MOUNTTYPE
/*
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" 0",ss); mytel_tcleval(tel,s);
*/


	/* --- TELESCOPE READY --- */
   res=astelco_put(tel,"SET TELESCOPE.POWER=1");
   sprintf(s,"after 100"); mytel_tcleval(tel,s);
   res=astelco_put(tel,"SET TELESCOPE.PARK=0");
	...
	/* --- Home --- */
	tel->latitude=43.2;
	sprintf(s,"SET TELESCOPE.CONFIG.LOCAL.LATITUDE = %f",tel->latitude);
   res=astelco_put(tel,s);
   res=astelco_put(tel,"SET TELESCOPE.CONFIG.LOCAL.LONGITUDE = 6.23");
   res=astelco_put(tel,"SET TELESCOPE.CONFIG.LOCAL.HEIGHT = 650");
      ...
      /* A loop is actived until the telescope is stopped */
		if (tel->blockingmethod==0) {
			...
			while (1==1) {
   			time_in++;
				sprintf(s,"after 350"); mytel_tcleval(tel,s);
				astelco_movingdetect(tel,1,&sepangledeg);
      ...
      /* A loop is actived until the telescope is stopped */
		if (tel->blockingmethod==0) {
			...
			while (1==1) {
   			time_in++;
				sprintf(s,"after 350"); mytel_tcleval(tel,s);
				astelco_movingdetect(tel,0,&sepangledeg);
				if (sepangledeg<sepangledeglim) { break; }
				if (time_in>=time_out) {break;}
			}
		}
...
int mytel_home_set(struct telprop *tel,double longitude,char *ew,double latitude,double altitude)
{
   ...
   tel->latitude=latitude;
	sprintf(s,"SET TELESCOPE.CONFIG.LOCAL.LATITUDE = %f",tel->latitude);
   res=astelco_put(tel,s);
	sprintf(s,"SET TELESCOPE.CONFIG.LOCAL.LONGITUDE = %f",longitude);
   res=astelco_put(tel,s);
	sprintf(s,"SET TELESCOPE.CONFIG.LOCAL.HEIGHT = %f",altitude);
   res=astelco_put(tel,s);
   return 0;
}
...
{
   astelco_hadec_coord(tel,result);
   return 0;
}
...
int astelco_read(struct telprop *tel,char *res)
{
   char s[2048],c[]="\\%c";
	...
	if (tel->type==0) {
		sprintf(s,"regsub -all \\# \"%s\" \"\" a ; regsub -all \\; \"$a\" \"\" s ; split $s ,",Tcl_GetStringResult(tel->interp));
	} else {
		sprintf(s,"regsub -all \\# \"%s\" \"\" a ; regsub -all \\; \"$a\" \"\" s ; split $s \"[format %s 0x0D]\"",Tcl_GetStringResult(tel->interp),c);
	}
	if (mytel_tcleval(tel,s)==1) {
		return 1;
	}
...
*/
{
   ...
   sprintf(s,"lindex \"%s\" 0",ss); mytel_tcleval(tel,s);
	octet=atoi(Tcl_GetStringResult(tel->interp));
...
*/
{
   ...
   res=astelco_read(tel,s);
   /* --- function COORDS --- */
   res=astelco_put(tel,"GET POSITION.EQUATORIAL.RA_J2000");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" 0",ss); mytel_tcleval(tel,s);
	ra=atof(Tcl_GetStringResult(tel->interp));
   res=astelco_put(tel,"GET POSITION.EQUATORIAL.DEC_J2000");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" 0",ss); mytel_tcleval(tel,s);
	dec=atof(Tcl_GetStringResult(tel->interp));
   /* --- --- */
...
*/
{
   ...
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" 0",ss); mytel_tcleval(tel,s);
	ha=atof(Tcl_GetStringResult(tel->interp));
   res=astelco_put(tel,"GET POSITION.EQUATORIAL.DEC_J2000");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" 0",ss); mytel_tcleval(tel,s);
...
*/
{
   ...
   res=astelco_read(tel,s);
   /* --- function COORDS --- */
   res=astelco_put(tel,"#25;");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" %d",ss,khara); mytel_tcleval(tel,s);
	ra1=15.*atof(Tcl_GetStringResult(tel->interp));
   sprintf(s,"lindex \"%s\" 2",ss); mytel_tcleval(tel,s);
	dec1=atof(Tcl_GetStringResult(tel->interp));
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   /* --- Vide le buffer --- */
   res=astelco_read(tel,s);
   /* --- function COORDS --- */
   ...
   res=astelco_read(tel,ss);
   sprintf(s,"lindex \"%s\" %d",ss,khara); mytel_tcleval(tel,s);
	ra2=15.*atof(Tcl_GetStringResult(tel->interp));
   sprintf(s,"lindex \"%s\" 2",ss); mytel_tcleval(tel,s);
	dec2=atof(Tcl_GetStringResult(tel->interp));
...
int astelco_goto(struct telprop *tel)
{
   ...
   res=astelco_read(tel,s);
   /* --- function SLEW --- */
   res=astelco_put(tel,"SET OBJECT.TYPE=3");
	sprintf(s,"SET OBJECT.EQUATORIAL.RA=%f",tel->ra0);
   res=astelco_put(tel,s);
	sprintf(s,"SET OBJECT.EQUATORIAL.DEC=%f",tel->dec0);
   res=astelco_put(tel,s);
	sprintf(s,"SET OBJECT.EQUATORIAL.EQUINOX=%f",epoch);
   res=astelco_put(tel,s);
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_put(tel,"SET POINTING.TRACK=1");
   /* --- --- */
   return 0;
}
...
int astelco_hadec_goto(struct telprop *tel)
{
   ...
   res=astelco_put(tel,s);
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   res=astelco_put(tel,"SET POINTING.TRACK=0");
   /* --- --- */
   return 0;
}
...
int astelco_initzenith(struct telprop *tel)
{
   ...
   res=astelco_read(tel,s);
   /* --- function TRACK --- */
   res=astelco_put(tel,"#14,0,0,0,0;");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   /* --- function ZENITH --- */
   res=astelco_put(tel,"#10;");
   sprintf(s,"after %d",tel->tempo); mytel_tcleval(tel,s);
   /* --- --- */
   /* --- function SLEW --- */
   res=astelco_put(tel,"#12;");
   return 0;
}
