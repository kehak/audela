...
int cmdTelStatus(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   struct telprop *tel;
   tel = (struct telprop *)clientData;
   res=astelco_stat(tel,ligne,bits);
   if (res==1) {
      result = TCL_ERROR;
   } else {
      Tcl_SetResult(interp,ligne,TCL_VOLATILE);
...
int cmdTelDFMFocus(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   if (argc>=3) {
      ...
      sprintf(ligne,"#27,%f;",value);
		...
		foc_analog=atof(Tcl_GetStringResult(tel->interp));
		sprintf(s,"lindex \"%s\" 6",ss); mytel_tcleval(tel,s);
		foc_digit=atof(Tcl_GetStringResult(tel->interp));
...
int cmdTelDFMTracking(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[]) {
   ...
   if (argc>2) {
	...
	tracking_ra=atof(Tcl_GetStringResult(tel->interp));
	sprintf(s,"lindex \"%s\" 11",ss); mytel_tcleval(tel,s);
	tracking_dec=atof(Tcl_GetStringResult(tel->interp));
