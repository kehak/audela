...
//
int cmdVisuBuf(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuClear(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuCut(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuDisp(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuImage(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuMirrorX(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuMirrorY(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuMode(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuPal(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuPalDir(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuWindow(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuThickness(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
int cmdVisuZoom(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[]);
...
int CmdVisu(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
   char s[1024],ss[50];
   int result = TCL_OK;
   int k;
...
int cmdVisuPal(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
   ...
   if((argc!=2)&&(argc!=3)) {
      ...
      if(strcmp(argv[2],"grey")==0) {
         ...
         // je traite les noms de repertoires contenant des caracteres accentues
         sprintf(ligne,"encoding convertfrom identity {%s}",argv[2]);
         Tcl_Eval(interp,ligne);
         res = ((CVisu*)clientData)->CreatePaletteFromFile(Tcl_GetStringResult(interp));
      }
      if (res != 0) {
         switch (res) {
         case ELIBSTD_NO_MEMORY_FOR_DISPLAY:
            ...
            break;
         default:
            Tcl_SetResult(interp,"can't find palette file",TCL_VOLATILE);
            result = TCL_ERROR;
            break;
         }
      } else {
...
int cmdVisuPalDir(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
   ...
   if((argc!=2)&&(argc!=3)) {
      ...
      sprintf(ligne,"encoding convertfrom identity {%s}",argv[2]);
      Tcl_Eval(interp,ligne);
      visu->SetPaletteDir(Tcl_GetStringResult(interp));
      free(ligne);
   }
   return result;
}
...
int cmdVisuCut(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
   ...
   if((argc!=2)&&(argc!=3)) {
      ...
      buffer = abaudela::IBuffer::getInstance(visu->bufnum);
      if (buffer != NULL ) {
