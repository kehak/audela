...
*/
   ...
   //  13   "snint" 
   p[13] = (float) (p[7] / sqrt( p[7] + npt * a[1] ));
...
// @param data
// param na  inutilisé
void fgauss3(float x, float a[], float *y, float dyda[], int )
{
...
// @param data
// param na  inutilisé
void fgauss1_2d(float x1, float x2, float a[], float *y, float dyda[], int )
{
	float ex;
	float ga,gb,gc;
   ...
   ga = (float) ( pow(cos(a[7]), 2) * pow(a[5], -2) / 2 + pow(sin(a[7]), 2) * pow(a[6], -2) / 2 );
   gb = (float) (-sin(2 * a[7]) * pow(a[5], -2) / 4 + sin(2 * a[7]) * pow(a[6], -2) / 4 );
   gc = (float) ( pow(sin(a[7]), 2) * pow(a[5], -2) / 2 + pow(cos(a[7]), 2) * pow(a[6], -2) / 2 );
   ex = (float) (exp(- ga * pow(x1 - a[3], 2) - 2 * gb * (x1 - a[3]) * (x2 - a[4]) - gc * pow(x2 - a[4], 2)) );
   ...
   *y = a[1] + a[2] * ex;
   ...
   dyda[5] = (float) ( a[2] * (pow(cos(a[7]), 2) * pow(x1 - a[3], 2) * pow(a[5], -3)
             - sin(2 * a[7]) * (x1 - a[3]) * (x2 - a[4]) * pow(a[5], -3) 
             + pow(sin(a[7]), 2) * pow(x2 - a[4], 2) * pow(a[5], -3)) * ex );
   dyda[6] = (float) (a[2] * (pow(sin(a[7]), 2) * pow(a[6], -3) * pow(x1 - a[3], 2)
             + sin(2 * a[7]) * pow(a[6], -3) * (x1 - a[3]) * (x2 - a[4]) 
             + pow(cos(a[7]), 2) * pow(a[6], -3) * pow(x2 - a[4], 2)) * ex );
   dyda[7] = (float)( a[2] * (-(-cos(a[7]) * pow(a[5], -2) * sin(a[7]) + sin(a[7])
   ...
   */
   ...
   /* itst < 4 : mode high precision */
      ...
      */
      ...
      mean = mean / (float) pow((float)npt,2) ;
   ...
   a[1] = sky;
	...
	for (i=1;i<=MA;i++) uncertainties[i] = (float) sqrt(covar[i][i]);
