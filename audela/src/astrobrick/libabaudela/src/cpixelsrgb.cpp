...
#include <string.h>     // memecpy, strcpy
...
CPixelsRgb::CPixelsRgb(int width, int height, TPixelFormat pixelFormat, void * pixels, int reverseX, int reverseY)
{
   ...
   if(pix==0) {
      throw CError(ELIBSTD_NO_MEMORY_FOR_PIXELS, "CPixelsRgb malloc" );
   }
   ...
   if( pixels != 0 ) {
      ...
      default :
         free(pix);
         throw CError(ELIBSTD_NO_MEMORY_FOR_PIXELS, "CPixelsRgb unkknown pixelFormat %d", pixelFormat );
         break;
      }
   }
 ...
 * @param   pixelSize   : Nombre de couleurs (3 ou 4)
 * @param   offset      : offset des couleurs
 *                -   offset[0] offet du rouge
 *                -  offset[1] offet du vert
 *                -  offset[2] offet du bleu
 *                -  offset[3] offet du alpha
 * @param   pitch       : non utilise
 * @param   pixels      : pointeur des donnees FORMAT_BYTE (un octet par couleur).
 * @return  Genere une exception CError en cas d'erreur.
 * Side effects:
 *    creation d'un objet de la classe CPixelsRgb
 *----------------------------------------------------------------------
 */

CPixelsRgb::CPixelsRgb(int width, int height, int pixelSize, int offset[4], int , unsigned char * pixels)
{
   long size;
   long base;
   long reverse = 1;
   ...
   if( pixels != NULL ) {
      if( pixelSize == 3 && offset[0] == 0 &&  offset[1] == 1 && offset[2] == 2 ) {
         ...
         free(pix);
         throw CError(ELIBSTD_NOT_IMPLEMENTED, "pixelSize = %d", pixelSize);
      }
   } else {
...
CPixelsRgb::CPixelsRgb(int width, int height, TPixelFormat pixelFormat, void *pixelsR, void *pixelsG, void *pixelsB) {
   ...
   if( pixelsR == NULL || pixelsG==NULL || pixelsB == NULL ) {
      throw CError(ELIBSTD_NO_MEMORY_FOR_PIXELS, "CPixelsRgb pixelsR = pixelsG = pixelsB = NULL");
   }
   ...
   pix = (TYPE_PIXELS_RGB*)malloc(size * sizeof(TYPE_PIXELS_RGB));
   if(pix==NULL) {
      throw CError(ELIBSTD_NO_MEMORY_FOR_PIXELS, "CPixelsRgb pix = NULL");
   }
   ...
   switch( pixelFormat ) {
       ...
       case FORMAT_UNKNOWN:
          throw CError(CError::ErrorInput, "undefined pixel format %d", pixelFormat);
         break;
   }
}
...
// @param x2
// @param width
void CPixelsRgb::BinX(int , int , int )
{
...
// @param y2
// @param height
void CPixelsRgb::BinY(int , int , int )
{
   throw CErrorAudela(ELIBSTD_NOT_IMPLEMENTED);
...
void CPixelsRgb::getPixels(int x1, int y1, int x2, int y2 , TPixelFormat pixelFormat, TColorPlane plane, void* pixels) {
   ...
   default :
      throw CErrorAudela(ELIBSTD_NO_MEMORY_FOR_PIXELS);
      break;
...
void CPixelsRgb::getPix(int *plane, TYPE_PIXELS *val1,TYPE_PIXELS *val2,TYPE_PIXELS *val3,int x, int y)
{
   ...
   if((x<0)||(x>=naxis1)) {
      throw CErrorAudela( ELIBSTD_X1X2_NOT_IN_1NAXIS1);
   } else if ((y<0)||(y>=naxis2)) {
      throw CErrorAudela( ELIBSTD_Y1Y2_NOT_IN_1NAXIS2);
   }
...
*/
void CPixelsRgb::getPixelsPointer(TYPE_PIXELS **) {
      throw CErrorAudela(ELIBSTD_NOT_IMPLEMENTED);
}
             ...
             unsigned char *palette[3], unsigned char *ptr)
{
   ...
   for(j=y1;j<=y2; j++) {
	   int orgww = x2 - x1 + 1; // Largeur de la fenetre au depart
	   //int orgwh = y2 - y1 + 1; // Hauteur ...
...
//   float fsbBlue = (float)locutBlue;
   ...
   for(j=y1;j<=y2; j++) {
      int orgww;        // original window width
      //int orgwh;        // original window height
 ...
 *----------------------------------------------------------------------
 */
bool CPixelsRgb::isPixelsReady(void) {
...
// param y0
// parma angle
void CPixelsRgb::Rot(float , float , float )
{
   throw CErrorAudela(ELIBSTD_NOT_IMPLEMENTED);
   /*
   int msg, datatype;
...
void CPixelsRgb::setPix(TColorPlane plane, TYPE_PIXELS val,int x, int y)
{
   ...
   case PLANE_GREY :
      ...
      *(pix+(y*naxis1+x)*naxis+2) = (unsigned char)  val;
      break;
   default:
      throw CError(CError::ErrorInput, "plane not authorized plane=%d", plane);
   }
...
void CPixelsRgb::Sub(CPixels * subPixels, float offset) {
   ...
   if ( this->getWidth() !=  subPixels->getWidth() && this->getHeight() != subPixels->getHeight() ) {
      throw CError(CError::ErrorGeneric, "Image size (%d,%d) is different from current image size (%d,%d)",
         subPixels->getWidth(), subPixels->getHeight(), this->getWidth(), this->getHeight() );
   }

   if ( subPixels->GetPlanes() != this->GetPlanes() ) {
      throw CError(CError::ErrorGeneric, "Image planes (%d) is different from current image plane(%d)",
...
// param x2
// param y2
void CPixelsRgb::Window(int , int , int , int )
{
   throw CErrorAudela(ELIBSTD_NOT_IMPLEMENTED);
   /*
   int temp;
