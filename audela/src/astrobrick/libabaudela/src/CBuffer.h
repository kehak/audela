...
public:
   ...
   static CBuffer * getDevice(int bufNo);
   void copyTo(IBuffer* dest);
   void copyFrom(IFitsKeywords*hdr, TColorPlane plane, TYPE_PIXELS*pix);
   void copyKwdFrom(IBuffer *org);
   ...
   void freeBuffer(TFreeBuffer keep_keywords);
   ...
   // pixels
   TPixelClass getPixelClass(const char * className);
   TPixelCompression getPixelCompression(const char *);
   TPixelFormat getPixelFormat(const char * formatName);
   TColorPlane getColorPlane(const char * planeName);
   ...
   void loadFits(const char *filename);
   void load3d  (const char *filename,int iaxis3);
   
   int  getNaxis();
   int  getWidth();
   bool isPixelsReady(void) ;
   ...
   */
   ...
   void saveRawFile(const char *filename);
   ...
   void Sub(IBuffer* subBuffer, float offset);
   ...
   void UnifyBg();
   void SubStars(const char *fileName, int indexcol_x, int indexcol_y, int indexcol_bg, double radius, double xc_exclu, double yc_exclu, double radius_exclu,int *n);
