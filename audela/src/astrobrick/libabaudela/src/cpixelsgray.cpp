...
void CPixelsGray::BinY(int y1, int y2, int height)
{
   ...
   if (height < 0)
   {
         throw CErrorAudela( ELIBSTD_HEIGHT_POSITIVE );
...
void CPixelsGray::getPixels(int x1, int y1, int x2, int y2, TPixelFormat pixelFormat, TColorPlane , void* pixels) {
   ...
   int x, y;
      ...
      case FORMAT_FLOAT:
         ...
         break;
      default:
         throw CErrorAudela(ELIBSTD_PIXEL_FORMAT_UNKNOWN);
         break;
...
void CPixelsGray::getPixelsReverse(int x1, int y1, int x2, int y2, TPixelFormat pixelFormat, TColorPlane , void* pixels) {
   int width  = x2-x1+1;
   int x, y;
   TYPE_PIXELS *ptr, *out;
 ...
 */
bool CPixelsGray::isPixelsReady(void) {
...
void CPixelsGray::UnifyBg()
{
   ...
   if (matbg==NULL) throw CErrorAudela(ELIBSTD_NO_MEMORY_FOR_PIXELS);
   ...
   if (lines==NULL) throw CErrorAudela(ELIBSTD_NO_MEMORY_FOR_PIXELS);

   // columns des centres des fenetres
   columns = (int*)malloc(nbCol * sizeof(int));
   if (columns==NULL) throw CErrorAudela(ELIBSTD_NO_MEMORY_FOR_PIXELS);
   ...
   for(x1=0; x1 < naxis1 ; x1 +=wRect) {
      ...
      for(y1=0; y1 < naxis2 ; y1 +=hRect) {
         ...
         if (pixel==NULL) throw CErrorAudela(ELIBSTD_NO_MEMORY_FOR_PIXELS);
