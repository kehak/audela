...
/* jj     : valeur du jour julien a convertir                              */
/* *chaine: code d'equinoxe correspondant                                  */
/* ref https://en.wikipedia.org/wiki/Epoch_%28astronomy%29#Julian_years_and_J2000 */
/***************************************************************************/
{
   double eps=1e-3,eps2=1e-4,a;
   char chaine0[80],format[80];
	if (year_type==0) {
		year_type=1; // J
		if ((fabs(jj-2415020.3135)<eps)||(fabs(jj-2433282.4235)<eps)) {
			year_type=2; // B
		}
	}
	if (year_type==1) {
	...
	if (year_type==2) {
		// besselian
		a = 1900.0 + (jj - 2415020.31352) / 365.242198781;
	   strcpy(chaine0,"B");
	}
	if (nb_subdigit>0) {
		sprintf(format,"%.%df",nb_subdigit);
		sprintf(chaine,format,chaine0,a);
	} else {
		sprintf(chaine,"%s%d",chaine0,(int)a);
	}
}
